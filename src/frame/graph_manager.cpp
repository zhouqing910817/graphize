...
class NodeIdGen {
private:
std::atomic<int> node_id_gen;
public:
NodeIdGen() {
    node_id_gen.store(0);
}
int allocate_node_id() {
    int id = node_id_gen.fetch_add(1);
    return id;
}
int get_cur_node_id() {
    return node_id_gen.load();
}
};
bool GraphManager::init(const std::string& file_path) {
    ...
    for (const std::string & graph_name : graph_name_vec) {
        ...
        NodeIdGen node_id_gen;
        auto create_service_if_not_exist = [&node_id_gen, &nodes_map]
        (const std::string name, std::vector<std::pair<std::string, std::string>>& graph_node_vec) ->Node* {
            ...
            if (!service_fac) {
                LOG(FATAL) << "unregistered class: [" << clazz << "]";
                return nullptr;
            }
            auto service_node = service_fac->create();
            service_node->init(node_config);
            service_node->set_name(name);
            service_node->id = node_id_gen.allocate_node_id();
            service_node->service_context_factory = service_fac; // to create service context
        ...
        graph->max_node_id = node_id_gen.get_cur_node_id();
